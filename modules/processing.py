...
def txt2img_image_conditioning(sd_model, x, width, height):
    ...
    if sd_model.model.conditioning_key in {"hybrid", "concat"}:
        ...
        image_conditioning = torch.zeros(x.shape[0],
                                         3,
                                         height,
                                         width,
                                         device=x.device)
...
class StableDiffusionProcessing:
    ...
    def txt2img_image_conditioning(self, x, width=None, height=None):
        ...
        return txt2img_image_conditioning(self.sd_model, x, width
                                          or self.width, height or self.height)
    ...
    def depth2img_image_conditioning(self, source_image):
        ...
        conditioning = 2.0 * (conditioning - depth_min) / (depth_max -
                                                           depth_min) - 1.0
        return conditioning
    ...
    def inpainting_image_conditioning(self,
                                      source_image,
                                      latent_image,
                                      image_mask=None):
        ...
        if image_mask is None:
            conditioning_mask = source_image.new_ones(1, 1,
        ...
        image_conditioning = torch.cat([conditioning_mask, conditioning_image],
                                       dim=1)
    ...
    def img2img_image_conditioning(self,
                                   source_image,
                                   latent_image,
                                   image_mask=None):
...
class Processed:
    ...
    def js(self):
        obj = {
            ...
            "seed_resize_from_h":
            self.seed_resize_from_h,
            "denoising_strength":
            self.denoising_strength,
            "extra_generation_params":
            self.extra_generation_params,
            "index_of_first_image":
            self.index_of_first_image,
            "infotexts":
            self.infotexts,
            "styles":
            self.styles,
...
def create_random_tensors(
    ...
    if (p is not None and p.sampler is not None
            and (len(seeds) > 1 and opts.enable_batch_seeds
    ...
    for i, seed in enumerate(seeds):
        noise_shape = (shape if seed_resize_from_h <= 0
                       or seed_resize_from_w <= 0 else
                       (shape[0], seed_resize_from_h // 8,
                        seed_resize_from_w // 8))
...
def create_infotext(
    ...
    generation_params = {
        ...
        getattr(
            p,
            "sd_model_hash",
            None if not opts.add_model_hash_to_info
            or not shared.sd_model.sd_model_hash else
            shared.sd_model.sd_model_hash,
        ),
        "Model": (None if not opts.add_model_name_to_info
                  or not shared.sd_model.sd_checkpoint_info.model_name else
        ...
        getattr(p, "inpainting_mask_weight",
                shared.opts.inpainting_mask_weight)
        if p.is_using_inpainting_conditioning else None,
        "Clip skip":
        None if clip_skip <= 1 else clip_skip,
        "ENSD":
        None if opts.eta_noise_seed_delta == 0 else opts.eta_noise_seed_delta,
        "Init image hash":
    ...
    negative_prompt_text = ("\nNegative prompt: " +
                            ...
                            if p.all_negative_prompts[index] else "")
...
def process_images(p: StableDiffusionProcessing) -> Processed:
    ...
    try:
        ...
        if (sd_models.checkpoint_alisases.get(
                p.override_settings.get("sd_model_checkpoint")) is None):
...
def process_images_inner(p: StableDiffusionProcessing) -> Processed:
    ...
    if os.path.exists(
            cmd_opts.embeddings_dir) and not p.do_not_reload_embeddings:
    ...
    with torch.no_grad(), p.sd_model.ema_scope():
        ...
        for n in range(p.n_iter):
            ...
            if n == 0:
                with open(os.path.join(paths.data_path, "params.txt"),
                          "w",
            ...
            if not shared.opts.dont_fix_second_order_samplers_schedule:
                try:
                    step_multiplier = (2 if sd_samplers.all_samplers_map.get(
                        p.sampler_name).aliases[0] in [
                            "k_dpmpp_2s_a",
                            "k_dpmpp_2s_a_ka",
                            "k_dpmpp_sde",
                            "k_dpmpp_sde_ka",
            ...
            ) if devices.unet_needs_upcast else devices.autocast():
            ...
            x_samples_ddim = [
                decode_first_stage(
                    p.sd_model,
                    samples_ddim[i:i +
                                 1].to(dtype=devices.dtype_vae))[0].cpu()
            ...
            for i, x_sample in enumerate(x_samples_ddim):
                ...
                if p.color_corrections is not None and i < len(
                        p.color_corrections):
                    if (opts.save and not p.do_not_save_samples
                    ...
                    image = apply_color_correction(p.color_corrections[i],
                                                   image)
        ...
        index_of_first_image = 0
        unwanted_grid_because_of_img_count = (len(output_images) < 2
                                              and opts.grid_only_if_multiple)
        if ((opts.return_grid or opts.grid_save) and not p.do_not_save_grid
...
class StableDiffusionProcessingTxt2Img(StableDiffusionProcessing):
    ...
    def init(self, all_prompts, all_seeds, all_subseeds):
        if self.enable_hr:
            if (opts.use_old_hires_fix_width_height
                    and self.applied_old_hires_behavior_to
            ...
            if self.hr_resize_x == 0 and self.hr_resize_y == 0:
                ...
                self.extra_generation_params[
                    "Hires resize"] = f"{self.hr_resize_x}x{self.hr_resize_y}"
                ...
                if self.hr_resize_y == 0:
                    ...
                    if src_ratio < dst_ratio:
                        self.hr_upscale_to_x = self.hr_resize_x
                        self.hr_upscale_to_y = (self.hr_resize_x *
                                                self.height // self.width)
                    else:
                        self.hr_upscale_to_x = (self.hr_resize_y *
                                                self.width // self.height)
                        self.hr_upscale_to_y = self.hr_resize_y

                    self.truncate_x = (self.hr_upscale_to_x -
                                       target_w) // opt_f
                    self.truncate_y = (self.hr_upscale_to_y -
                                       target_h) // opt_f

            # special case: the user has chosen to do nothing
            if (self.hr_upscale_to_x == self.width
            ...
            if self.hr_second_pass_steps:
                self.extra_generation_params[
                    "Hires steps"] = self.hr_second_pass_steps

            if self.hr_upscaler is not None:
                self.extra_generation_params[
                    "Hires upscaler"] = self.hr_upscaler
    ...
    def sample(
        ...
        self.sampler = sd_samplers.create_sampler(self.sampler_name,
                                                  self.sd_model)

        latent_scale_mode = (
            shared.latent_upscale_modes.get(self.hr_upscaler, None) if
            self.hr_upscaler is not None else shared.latent_upscale_modes.get(
        ...
        def save_intermediate(image, index):
            ...
            if (not opts.save or self.do_not_save_samples
                    or not opts.save_images_before_highres_fix):
                return

            if not isinstance(image, Image.Image):
                image = sd_samplers.sample_to_image(image,
                                                    index,
                                                    approximation=0)
        ...
        if latent_scale_mode is not None:
            ...
            if (getattr(self, "inpainting_mask_weight",
                        shared.opts.inpainting_mask_weight) < 1.0):
            ...
            lowres_samples = torch.clamp((decoded_samples + 1.0) / 2.0,
                                         min=0.0,
                                         max=1.0)
        ...
        self.sampler = sd_samplers.create_sampler(img2img_sampler_name,
                                                  self.sd_model)
...
class StableDiffusionProcessingImg2Img(StableDiffusionProcessing):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.image_cfg_scale: float = (image_cfg_scale
                                       if shared.sd_model.cond_stage_key
                                       == "edit" else None)
    ...
    def init(self, all_prompts, all_seeds, all_subseeds):
        ...
        if image_mask is not None:
            ...
            if self.inpaint_full_res:
                ...
                crop_region = masking.expand_crop_region(
                    crop_region, self.width, self.height, mask.width,
                    mask.height)
                x1, y1, x2, y2 = crop_region

                mask = mask.crop(crop_region)
                image_mask = images.resize_image(2, mask, self.width,
                                                 self.height)
                ...
                np_mask = np.clip((np_mask.astype(np.float32)) * 2, 0,
                                  255).astype(np.uint8)
        ...
        add_color_corrections = (opts.img2img_color_correction
                                 and self.color_corrections is None)
        ...
        for img in self.init_images:
            ...
            if crop_region is None and self.resize_mode != 3:
                image = images.resize_image(self.resize_mode, image,
                                            self.width, self.height)
        ...
        if len(imgs) == 1:
            batch_images = np.expand_dims(imgs[0],
                                          axis=0).repeat(self.batch_size,
                                                         axis=0)
            if self.overlay_images is not None:
                self.overlay_images = self.overlay_images * self.batch_size

            if self.color_corrections is not None and len(
        ...
        if image_mask is not None:
            ...
            latmask = np.moveaxis(np.array(latmask, dtype=np.float32), 2,
                                  0) / 255
            latmask = latmask[0]
            latmask = np.around(latmask)
            latmask = np.tile(latmask[None], (4, 1, 1))

            self.mask = (torch.asarray(1.0 - latmask).to(shared.device).type(
                self.sd_model.dtype))
            self.nmask = (torch.asarray(latmask).to(shared.device).type(
                self.sd_model.dtype))
    ...
    def sample(
        ...
        if self.initial_noise_multiplier != 1.0:
            self.extra_generation_params[
                "Noise multiplier"] = self.initial_noise_multiplier
