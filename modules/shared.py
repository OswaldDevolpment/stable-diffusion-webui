...
cmd_opts.disable_extension_access = (
    cmd_opts.share or cmd_opts.listen
    or cmd_opts.server_name) and not cmd_opts.enable_insecure_extension_access
...
) = (devices.cpu if any(y in cmd_opts.use_cpu
                        for y in [x, "all"]) else devices.get_optimal_device()
...
class State:
    ...
    def set_current_image(self):
        ...
        if (self.sampling_step - self.current_image_sampling_step
...
options_templates = options_section(
    ...
    {
        "samples_save":
        OptionInfo(True, "Always save all generated images"),
        "samples_format":
        OptionInfo("png", "File format for images"),
        "samples_filename_pattern":
        OptionInfo("", "Images filename pattern", component_args=hide_dirs),
        ...
        "grid_save":
        OptionInfo(True, "Always save all generated image grids"),
        "grid_format":
        OptionInfo("png", "File format for grids"),
        ...
        OptionInfo(True, "Do not save grids consisting of one picture"),
        ...
        "save_mask":
        OptionInfo(False, "For inpainting, save a copy of the greyscale mask"),
        "save_mask_composite":
        OptionInfo(False, "For inpainting, save a masked composite"),
        "jpeg_quality":
        OptionInfo(
            80,
        ...
        "webp_lossless":
        OptionInfo(False, "Use lossless compression for webp images"),
        "export_for_4chan":
        OptionInfo(
            True,
        ...
        OptionInfo(4.0, "File size limit for the above option, MB", gr.Number),
        "target_side_length":
        OptionInfo(
            4000,
        ...
        "img_max_size_mp":
        OptionInfo(200, "Maximum image size, in megapixels", gr.Number),
        ...
        "save_init_img":
        OptionInfo(False, "Save init images when using img2img"),
        "temp_dir":
        OptionInfo("",
...
options_templates.update(
    options_section(
        ...
        {
            "save_to_dirs":
            OptionInfo(True, "Save images to a subdirectory"),
            "grid_save_to_dirs":
            OptionInfo(True, "Save grids to a subdirectory"),
...
options_templates.update(
    options_section(
        ("upscaling", "Upscaling"),
        {
            ...
            OptionInfo(
                8,
                "Tile overlap, in pixels for SCUNET upscalers. Low values = visible seam.",
                gr.Slider,
                {
                    "minimum": 0,
                    "maximum": 64,
...
options_templates.update(
    options_section(
        ...
        {
            "show_warnings":
            OptionInfo(False, "Show warnings in console."),
            "memmon_poll_rate":
            OptionInfo(
                8,
...
options_templates.update(
    options_section(
        ("training", "Training"),
        {
            ...
            OptionInfo("", "Filename word regex"),
            "dataset_filename_join_string":
            OptionInfo(" ", "Filename join string"),
            ...
            OptionInfo(False, "Enable tensorboard logging."),
            "training_tensorboard_save_images":
            OptionInfo(False, "Save generated images within tensorboard."),
...
options_templates.update(
    options_section(
        ("sd", "Stable Diffusion"),
        {
            ...
            OptionInfo(1, "Clip skip", gr.Slider, {
                "minimum": 1,
                "maximum": 12,
                "step": 1
            }),
            "upcast_attn":
            OptionInfo(False, "Upcast cross attention layer to float32"),
            "randn_source":
            OptionInfo(
                "GPU",
...
options_templates.update(
    options_section(
        ("interrogate", "Interrogate Options"),
        {
            ...
            OptionInfo(False, "Interrogate: keep models in VRAM"),
            ...
            OptionInfo(True, "Interrogate: deepbooru sort alphabetically"),
            "deepbooru_use_spaces":
            OptionInfo(False, "use spaces for tags in deepbooru"),
            "deepbooru_escape":
            OptionInfo(
                True,
...
options_templates.update(
    options_section(
        ("extra_networks", "Extra Networks"),
        {
            ...
            OptionInfo(0, "Card width for Extra Networks (px)"),
            "extra_networks_card_height":
            OptionInfo(0, "Card height for Extra Networks (px)"),
...
options_templates.update(
    options_section(
        ...
        {
            "return_grid":
            OptionInfo(True, "Show grid in results for web"),
            "return_mask":
            OptionInfo(
                False,
            ...
            "do_not_show_images":
            OptionInfo(False, "Do not show any images in results for web"),
            "add_model_hash_to_info":
            OptionInfo(True, "Add model hash to generation information"),
            "add_model_name_to_info":
            OptionInfo(True, "Add model name to generation information"),
            ...
            "font":
            OptionInfo("", "Font for image grids that have text"),
            "js_modal_lightbox":
            OptionInfo(True, "Enable full page image viewer"),
            ...
            OptionInfo(True, "Show generation progress in window title."),
            ...
            "keyedit_delimiters":
            OptionInfo(".,\/!?%^*;:{}=`~()", "Ctrl+up/down word delimiters"),
            "quicksettings":
            OptionInfo("sd_model_checkpoint", "Quicksettings list"),
            "hidden_tabs":
            OptionInfo(
                [],
            ...
            OptionInfo("", "Extra networks tab order"),
            "localization":
            OptionInfo(
                "None",
                ...
                gr.Dropdown,
                lambda: {
                    "choices": ["None"] + list(localization.localizations.keys(
                    ))
                },
...
options_templates.update(
    options_section(
        ...
        {
            "show_progressbar":
            OptionInfo(True, "Show progressbar"),
            "live_previews_enable":
            OptionInfo(True, "Show live previews of the created image"),
            "show_progress_grid":
            OptionInfo(
                True,
...
options_templates.update(
    options_section(
        ("sampler-params", "Sampler parameters"),
        {
            ...
            OptionInfo(0, "Eta noise seed delta", gr.Number, {"precision": 0}),
            "always_discard_next_to_last_sigma":
            OptionInfo(False, "Always discard next-to-last sigma"),
            "uni_pc_variant":
            OptionInfo(
                "bh1",
                "UniPC variant",
            ...
            OptionInfo(True, "UniPC lower order final"),
        },
    ))
...
options_templates.update(
    options_section(
        ...
        {
            "disabled_extensions":
            OptionInfo([], "Disable these extensions"),
            ...
            "sd_checkpoint_hash":
            OptionInfo("", "SHA256 hash of the current checkpoint"),
        },
    ))
...
class Options:
    ...
    def __setattr__(self, key, value):
        if self.data is None or key not in self.data and key not in self.data_labels:
            return super(Options, self).__setattr__(key, value)
        assert not cmd_opts.freeze_settings, "changing settings is disabled"

        info = opts.data_labels.get(key, None)
        comp_args = info.component_args if info else None
        if isinstance(comp_args, dict) and comp_args.get("visible",
                                                         True) is False:
            raise RuntimeError(
    ...
    def dumpjson(self):
        d = {
            k: self.data.get(k,
                             self.data_labels.get(k).default)
